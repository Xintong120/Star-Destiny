...
修复了选择流年后流年宫位名称不显示的问题，确保大限和流年运限均能正确展示命宫、兄弟宫等十二宫位名称。

#### 问题分析

在实现流年运限查看功能时，发现流年的十二宫名称（命宫、兄弟宫、夫妻宫等）未能正确显示。这主要是因为流年运限数据中缺少宫位名称数组。

#### 解决方案

我将在 HoroscopeAdapter.ts 的 formatDecadalInfo 函数中，补上与流年逻辑相仿的【大限十二宫名称计算逻辑】。这能确保无论是大限还是流年，适配器都会返回一个包含 palaceNames 数组的、格式统一的数据对象，从而解决大限宫位名称不显示的问题。

#### 更新内容

1. **添加大限宫位名称计算逻辑**：
   - 在 `formatDecadalInfo` 方法中，增加了大限十二宫位名称的动态计算
   - 基于流年地支决定流年命宫的位置
   - 再根据命主性别和年干的阴阳决定其他宫位的顺/逆时针排列

2. **明确排列方向**：
   - 根据紫微斗数的排盘规则 "阳男阴女顺行，阴男阳女逆行"
   - 通过判断生年天干的阴阳和命主性别，确定宫位排列方向

3. **统一数据结构**：
   - 确保大限和流年数据结构统一，都包含 `palaceNames` 属性
   - 使得ZiWeiGrid组件能够以相同的方式处理不同类型的运限数据
...
1. **功能完整性**：实现了流年宫位名称的显示，使得命盘信息更加完整
2. **用户体验改善**：用户现在可以清晰地看到流年影响的具体宫位
3. **一致性保证**：大限和流年数据结构的统一，简化了前端组件的处理逻辑
4. **代码复用**：使用相似的计算逻辑处理不同类型的运限，提高了代码复用性
5. **满足专业需求**：符合紫微斗数专业用户的使用习惯和预期